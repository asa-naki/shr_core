...
public:
     ...
     * @return 成功時true
     */
    virtual bool setPosition(uint8_t servo_id, uint16_t position, bool enable_torque = true, double timeout = 5.0) = 0;
...
public:
    ...
    bool setPosition(uint8_t servo_id, uint16_t position, bool enable_torque = true, double timeout = 5.0) override;
...
private:
     ...
     * @return コマンド長
     */
    size_t createModbusCommand(uint8_t slave_id, uint8_t function_code, uint16_t address, uint16_t value, uint8_t* command);
     ...
     * @return 応答長（失敗時-1）
     */
    int sendCommand(const uint8_t* cmd_bytes, size_t cmd_length, uint8_t* response, size_t max_response_length, bool expect_response = true);
...
public:
     ...
     * @return 成功時true
     */
    bool aimAt(uint8_t pan_servo_id, uint8_t tilt_servo_id, uint16_t pan_position, uint16_t tilt_position, double timeout = 5.0);
     ...
     * @return 成功時true
     */
    bool fire(uint8_t trigger_servo_id, uint16_t fire_position, uint16_t return_position, double fire_duration = 0.5);
    ...
    bool returnHome(uint8_t pan_servo_id, uint8_t tilt_servo_id, uint8_t trigger_servo_id, 
                   uint16_t pan_home = 2048, uint16_t tilt_home = 2048, uint16_t trigger_home = 2048);
     ...
     * @return 成功時true
     */
    bool getCurrentAim(uint8_t pan_servo_id, uint8_t tilt_servo_id, int32_t& pan_position, int32_t& tilt_position);
